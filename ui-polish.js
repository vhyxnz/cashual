...
function polishControls(){
  for(const button of document.querySelectorAll('button')){
    if(button.matches('.wallet-main,.profile-setting,.setting,.category-pill,.quick-category,.category-icon-option,.color-choice,.mobile-nav-add,.day,.data-action-row,.mobile-nav-choice,.expense-period,.wallet-card-actions button,.repeat-last,.streak-review,.detail-action,.reconcile-action,[data-wallet-privacy],[data-wallet-more],#themeBtn,#privacyBtn'))continue;

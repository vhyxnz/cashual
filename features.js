...
document.addEventListener('touchmove',event=>{if(!swipeStart)return;const touch=event.touches[0],dx=touch.clientX-swipeStart.x,dy=touch.clientY-swipeStart.y;if(Math.abs(dy)>Math.abs(dx)+10)return;swipeStart.dx=Math.max(-86,Math.min(86,dx));swipeStart.shell.style.setProperty('--swipe-x',`${swipeStart.dx}px`);swipeStart.shell.classList.add('swiping')},{passive:true});
document.addEventListener('touchend',()=>{if(!swipeStart)return;const {shell,dx}=swipeStart;shell.classList.remove('swiping');shell.style.setProperty('--swipe-x',Math.abs(dx)>46?`${Math.sign(dx)*76}px`:'0px');swipeStart=null},{passive:true});

...
document.addEventListener('click',event=>{const button=event.target.closest('[data-budget-delete]');if(!button)return;event.preventDefault();event.stopImmediatePropagation();const budget=state.budgets.find(b=>b.id===button.dataset.budgetDelete);if(!budget||!confirm(`Delete “${budget.name}”?`))return;state.budgets=state.budgets.filter(b=>b.id!==budget.id);save();editRecord=null;formDialog.close();render();toastMsg('Budget deleted')},true);

...
transactions=function(){
  ...
  return `<section class="expense-page"><div class="expense-intro"><div><p class="expense-eyebrow">Your spending</p><h2>Expenses</h2><p>Explore your activity by period, type, or category.</p></div><button class="primary-btn" data-open="expense" aria-label="Add expense" title="Add expense">${svg('add')}</button></div><div class="expense-periods" role="group" aria-label="Expense period">${[['daily','Day'],['weekly','Week'],['monthly','Month'],['yearly','Year']].map(([key,label])=>`<button class="expense-period ${expenseRange===key?'active':''}" data-expense-period="${key}" aria-pressed="${expenseRange===key}">${label}</button>`).join('')}</div><div class="expense-overview card"><div><span>Net spent ${labels[expenseRange]||'today'}</span><strong>${spent<0?'−':''}${cash(spent)}</strong></div><small>${items.filter(t=>t.type==='expense').length} expense${items.filter(t=>t.type==='expense').length===1?'':'s'} recorded</small></div><div class="expense-list-head"><div><h3>Activity</h3><small id="expenseCount">${visible.length} entr${visible.length===1?'y':'ies'}</small></div></div><div class="expense-controls"><label class="expense-search"><span class="sr-only">Search activity</span>${svg('search')}<input type="search" id="expenseSearch" placeholder="Search name, category, wallet" value="${escC(expenseSearch)}"></label><label class="expense-type"><span class="sr-only">Transaction type</span><select id="expenseTypeFilter"><option value="all" ${expenseTypeFilter==='all'?'selected':''}>All types</option><option value="expense" ${expenseTypeFilter==='expense'?'selected':''}>Expenses</option><option value="refund" ${expenseTypeFilter==='refund'?'selected':''}>Refunds</option><option value="income" ${expenseTypeFilter==='income'?'selected':''}>Income</option><option value="transfer" ${expenseTypeFilter==='transfer'?'selected':''}>Transfers</option></select></label></div><div class="expense-list transactions" id="txList">${txRows(visible)}</div></section>`;
};
